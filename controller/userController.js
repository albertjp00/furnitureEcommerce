...
const loginLoad = async (req, res) => {
  try {
    res.render("login", {
      redirect: req.query.redirect || "/user/home",
    });
  } catch (error) {
    console.log(error.message);
    res.render("error");
  ...
  try {
    ...
    if (user) {
      if (user.status === "Blocked") {
        ...
        res.redirect(redirect || "/user/home");
      } else {
...
const newUser = async (req, res) => {
  try {
    ...
    if (existingVerifiedUser) {
      res.render("signup", {
        message: "Email is already in use by an existing verified user.",
      });
      return;
    }
...
const cartAdd = async (req, res) => {
  try {
    ...
    if (!cartItem) {
      ...
      const existingProductIndex = cartItem.products.findIndex((product) =>
...
const cartLoad = async (req, res) => {
  try {
    ...
    res.render("cart", { product: cartItems, total });
  } catch (error) {
    console.log(error.message);
    res.render("error");
  }
...
const cartDelete = async (req, res) => {
  try {
    ...
    if (deletedCart) {
      ...
      res
        .status(404)
        .json({ message: "Cart not found or product not in the cart." });
    }
  } catch (error) {
    // Handle error
    res.render("error");
...
const checkout = async (req, res) => {
  try {
    ...
    const notUsedCoupons = coupons.filter(
      (coupon) =>
        !user.coupons.some((userCoupon) => userCoupon.code === coupon.code),
    );
...
const removeCoupon = async (req, res) => {
  try {
    ...
    const indexToRemove = user.coupons.findIndex(
    ...
    if (req.body.paymentMethod == "Cash On Delivery") {
      ...
      for (const cartProduct of cartItems.products) {
        ...
        if (productDetails) {
          ...
          console.log(
            "stockkk222------" + productDetails.stock,
            productDetails.count,
          );
        }
      }
...
const loadDetails = async (req, res) => {
  try {
    ...
    if (!order) {
      return res
        .status(404)
        .send("Order not found for this user with the specified product");
    }
...
const invoice = async (req, res) => {
  try {
    ...
    order.products.forEach((product, index) => {
      doc.text(
        `${index + 1}. Product: ${product.name}, Quantity: ${product.quantity}, Price: ${product.price}`,
      );
    });
    doc.moveDown();
...
const passwordCheck = async (req, res) => {
  try {
    ...
    if (req.body.password === userData.password) {
      ...
      return res.send(
        '<script>alert("Wrong Password. Try Again."); window.location="/user/profile";</script>',
      );
    }
  } catch (error) {
    console.error(error.message);
...
const passwordChange = async (req, res) => {
  try {
    ...
    if (req.body.password1 !== req.body.password2) {
      return res.redirect("/user/loadChange?error=PasswordMismatch");
    }
    ...
    return res.redirect("/user/profile?success=PasswordUpdated");
...
const addAddress = async (req, res) => {
  try {
    ...
    if (existingAddress) {
      return res.redirect("/user/toAddress?error=AddressExists");
    }
    ...
    return res.redirect("/user/profile?success=AddressAdded");

  } catch (error) {
    console.log(error.message);
    return res.redirect("/user/toAddress?error=ServerError");
  }
};
...
const addAddressCheckout = async (req, res) => {
  try {
    ...
    if (existingAddress) {
      return res.send(
        '<script>alert("Address already exists! Please try again."); window.location="/user/toAddress";</script>',
      );
    }
    ...
    if (
      error.code === 11000 &&
      error.keyPattern &&
      error.keyPattern.houseAddress
    ) {
      // The error code 11000 corresponds to a duplicate key error (unique constraint violation)
      return res.send(
        '<script>alert("Password not matching Please Try Again"); window.location="/user/profile";</script>',
      );
    }
...
const editAddress = async (req, res) => {
  try {
    ...
    if (existingAddress) {
        return res.redirect(`/user/loadEdit?id=${id}&error=AddressExists`);
    }
...
const addToWishlist = async (req, res) => {
  try {
    ...
    let wishItem = await Wishlist.findOne({ userId });
    ...
    if (!wishItem) {
      ...
      const exists = wishItem.products.find((p) =>
      ...
      wishItem.products.push({ productId });
    }
  ...
  try {
    ...
    if (deletedWish) {
      ...
      res
        .status(404)
        .json({
          message: "Wishlist not found or product not in the wishlist.",
        });
    }
  } catch (error) {
    console.log(error.message);
