...
const loadDash = async (req, res) => {
  try {
    ...
    monthlySalesData.forEach((month) => {
      const monthName = new Date(2000, month._id - 1).toLocaleString(
        "default",
        { month: "short" },
      );
...
const categoryEdit = async (req, res) => {
  try {
    ...
    if (categoryData) {
      res.render("page-categoryEdit", {
        category: categoryData,
        category1: categoryData1,
      });
...
const loadOrderDetails = async (req, res) => {
  try {
    ...
    res.render("page-orders-details", {
      order: order,
      product: products,
      address: address,
    });
...
const approveReturn = async (req, res) => {
  try {
    ...
    await order.save();
    console.log(
      "Details of the returned product:",
      order.products[productIndex],
    );
  ...
  try {
    ...
    if (existingCoupon) {
      ...
      return res.render("page-coupon", {
        message: "Error: Coupon with the same name,amount or code already exists.",
        coupon: couponData,
        totalPages,
        currentPage: page,
      });
    ...
    if (error.code === 11000 && error.keyPattern && error.keyPattern.name) {
      const coupondata = await Coupon.find();
      res.render("page-coupon", {
        message: "Error: Coupon already exists.",
        coupon: coupondata,
      });
    }
    if (error.name === "ValidationError") {
...
const updateCoupon = async (req, res) => {
  try {
    ...
    console.log(name, amount, code, existingCoupon);
    ...
    if (existingCoupon) {
      ...
      return res.render("page-coupon", {
        ...
        message: "Coupon with same name, code or amount already exists.",
      });
    }

    // ✅ Update if no duplicate
    await Coupon.findByIdAndUpdate(id, { name, amount, code }, { new: true });
...
const updateOffer = async (req, res) => {
  try {
    ...
    await Offer.findByIdAndUpdate(
      req.query.id,
      { offer: req.body.offer, category: req.body.category },
      { new: true },
    );
